...
export default {
  ...
  methods: {
    ...
    async pay() {
      ...
      // 循环定时器   每3秒发请求查看支付状态信息
      // 定时器设置后返回的是一个编号，是一个数字，我们使用变量存储的是这个数字
      // 定时器又是一个异步操作，所以会有对应得管理模块去管理 ，依赖的是编号
      // 清除定时器，其实是把这个编号的定时器任务清除，但是这个编号还是存在你之前保存的变量当中
      // 因此以后清除定时器  记得把保存编号的变量也去置为null,代表彻底清除
